...
export default function StudyDeck({ cards }: { cards: CardT[] }) {
  ...
  if (cards.length === 0) {
    return <p className="text-center text-slate-500">Inga kort än — lägg till ditt första nedan! 👇</p>;
  }
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      `}</style>
      <p className="text-sm text-slate-400">Kort {i + 1} / {order.length} — klicka på kortet för att vända! ✨</p>
        ...
        <div className={`qk-card3d relative h-72 w-full ${flipped ? "qk-flipped" : ""}`}>
          <div className="qk-face absolute inset-0 flex items-center justify-center rounded-3xl border border-indigo-500/40 bg-gradient-to-br from-indigo-600/30 via-slate-900 to-purple-600/30 p-8 shadow-2xl shadow-indigo-500/20">
            <div className="text-center">
              <p className="mb-2 text-xs uppercase tracking-widest text-indigo-300">Fråga</p>
          ...
          <div className="qk-face qk-back absolute inset-0 flex items-center justify-center rounded-3xl border border-emerald-500/40 bg-gradient-to-br from-emerald-600/30 via-slate-900 to-teal-600/30 p-8 shadow-2xl shadow-emerald-500/20">
            <div className="text-center">
              <p className="mb-2 text-xs uppercase tracking-widest text-emerald-300">Svar</p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        <button onClick={() => go(-1)} className="rounded-xl bg-slate-800 px-5 py-2 font-bold text-white hover:bg-slate-700">⬅️ Föregående</button>
        <button onClick={() => setFlipped((f) => !f)} className="rounded-xl bg-indigo-600 px-6 py-2 font-bold text-white hover:bg-indigo-500">🔄 Vänd</button>
        <button onClick={() => go(1)} className="rounded-xl bg-slate-800 px-5 py-2 font-bold text-white hover:bg-slate-700">Nästa ➡️</button>
        <button onClick={shuffle} className="rounded-xl bg-purple-600 px-5 py-2 font-bold text-white hover:bg-purple-500">🎲 Blanda</button>
      </div>
    </div>
  );
}