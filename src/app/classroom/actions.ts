...
export async function createClassroom(formData: FormData) {
  ...
  if (!profile || profile.role !== "TEACHER") return { ok: false };
...
export async function addCoTeacher(formData: FormData) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false };
  ...
  if (!room || room.ownerId !== user.id) return { ok: false };
  const teacher = await prisma.profile.findUnique({ where: { username } });
  if (!teacher || teacher.role !== "TEACHER") return { ok: false };
